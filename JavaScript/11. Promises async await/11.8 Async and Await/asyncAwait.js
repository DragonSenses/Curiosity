...
callable then method). The idea is that a third-party object may not be a 
promise, but promise-compatible: if it supports .then, that's enough to use
it with await.
...
/* If await gets a non-promise object with .then, it calls that method providing 
the built-in functions resolve and reject as arguments (just as it does for a 
regular Promise executor). Then await waits until one of them is called (in 
the example above it happens in the line (*)) and then proceeds with the result. */
...
/* The meaning is the same: it ensures that the returned value is a promise and 
enables await. */


/* Error handling */
/* If a promise resolves normally, then await promise returns the result. But 
in the case of a rejection, it throws the error, just as if there were a throw
...
/* In the case of an error, the control jumps to the catch block. We can also 
wrap multiple lines: */
{
  async function f() {
...
/* If we don't have try..catch, then the promise generated by the call of 
the async function f() becomes rejected. We can append .catch to handle it: */
{
  async function f() {
    let response = await fetch('http://no-such-url');
  }
...
(viewable in the console). We can catch such errors using a global 
unhandledrejection event handler as described in the chapter Error handling 
with promises. */

/* async/await and promise.then/catch */
/* When we use async/await, we rarely need .then, because await handles the 
waiting for us. And we can use a regular try..catch instead of .catch. That's 
usually (but not always) more convenient.
...
.then/catch to handle the final result or falling-through error, like in the 
