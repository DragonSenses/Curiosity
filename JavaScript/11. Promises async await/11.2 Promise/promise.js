...
/* The call .catch(f) is a just a shorthand, a complete analog of .then(null, f) */
...
There are important differences: 
  ...
  promise is successful or not. That's all right, as our task is usually to 
...
/* As you can see, the value returned by the first promise is passed through 
finally to the next then.

That's very convenient, because finally is not meant to process a promise 
result. As said, it's a place to do generic cleanup, no matter what the outcome was.

And here's an example of an error, for us to see how it's passed through 
finally to catch: */
{
  new Promise((resolve, reject) => {
...
/* 3. A finally handler also shouldn't return anything. If it does, the 
returned value is silently ignored.
...
To summarize:
  - A finally handler doesn't get the outcome of the previous handler (it has 
    no arguments). This outcome is passed through instead, to the next suitable handler.
  - If a finally handler returns something, it's ignored.
  - When finally throws an error, then the execution goes to the nearest error handler.

These features are helpful and make things work just the right way if we use 
finally how it's supposed to be used: for generic cleanup procedures.
*/
...
/* Note that this makes promises more powerful than the real life 
"subscription list" scenario. If the singer has already released their song 
and then a person signs up on the subscription list, they probably won't 
receive that song. Subscriptions in real life must be done prior to the event.

Promises are more flexible. We can add handlers any time: if the result is 
already there, they just execute. */


/* Example: loadScript */
/* Next, let's see more practical examples of how promises can help us 
...
The new function loadScript will not require a callback. Instead, it will 
create and return a Promise object that resolves when the loading is complete. 
...
/* 
Promises                                                  Callbacks
Promises allow us to do things in the natural order.      We must have a callback function at our disposal when
First, we run loadScript(script), and .then we write      calling loadScript(script, callback). In other words,
what to do with the result.                               we must know what to do with the result before loadScript is called.

We can call .then on a Promise as many times as we        There can be only one callback.
want. Each time, we're adding a new "fan", a new 
