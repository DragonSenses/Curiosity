...
/**
 * A decorator that returns a wrapper that when it’s called multiple times, 
 * it passes the call to f at maximum once per ms milliseconds.
 * 
 * Runs it not more often than given ms time. Good for regular updates 
...
function throttle(func, ms){
  ...
  let isThrottled = false, // Flag for Cooldown State
    savedArgs,  // save Arguments in memory
    savedThis;  // save Context in memory
...
/* A call to throttle(func, ms) returns wrapper. 
  ...
  that both the context and the arguments are equally important and should be 
  memorized. We need them simultaneously to reproduce the call.
