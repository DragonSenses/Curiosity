...
For example, getName() method accesses the private #name property and breaks 
after proxying: */
{
  class User {
    #name = "Guest";
...
In the call getName() the value of this is the proxied user, and it doesn't 
have the slot with private fields.
...
/* That said, the solution has drawbacks, as explained previously: it exposes 
the original object to the method, potentially allowing it to be passed further
and breaking other proxied functionality. */

/* Proxy != target */
/* The proxy and the original object are different objects. That's natural, right?

So if we use the original object as a key, and then proxy it, then the proxy 
can't be found: */
{
...
/* As we can see, after proxying we can't find user in the set allUsers, 
because the proxy is a different object. */

/* Proxies can't intercept a strict equality test === */
/* Proxies can intercept many operators, such as new (with construct), 
in (with has), delete (with deleteProperty) and so on.

But there's no way to intercept a strict equality test for objects. An object 
is strictly equal to itself only, and no other value.
...
What we can do is to wrap it into a revocable proxy, without any traps. 
Such a proxy will forward operations to object, and we can disable it at any moment.
...
/* A call to revoke() removes all internal references to the target object from
the proxy, so they are no longer connected.

Initially, revoke is separate from proxy, so that we can pass proxy around 
while leaving revoke in the current scope.

We can also bind revoke method to proxy by setting proxy.revoke = revoke.

Another option is to create a WeakMap that has proxy as the key and the 
corresponding revoke as the value, that allows to easily find revoke for a 
proxy: */
...
If a proxy object becomes "unreachable" (e.g. no variable references it any 
more), WeakMap allows it to be wiped from memory together with its revoke 
