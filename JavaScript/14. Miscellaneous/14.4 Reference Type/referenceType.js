...
(user.name == "Luna" ? user.hi : user.bye)(); // Error!
...
Then the method is immediately called with parentheses (). But it 
...
This doesn't (evaluated method):
  (user.name == "Luna" ? user.hi : user.bye)();
...
/* Here hi = user.hi puts the function into the variable, and then on the last 
line it is completely standalone, and so there's no this.
...
The Reference Type is a "specification type". We can't explicitly use it, but 
it is used internally by the language. */
...
information about the object and its method, and can set the right this (user in this case).

Reference type is a special "intermediary" internal type, with the purpose to 
pass information from dot . to calling parentheses ().

Any other operation like assignment hi = user.hi discards the reference type 
as a whole, takes the value of user.hi (a function) and passes it on. So any 
...
obj['method']() syntax (they do the same here). There are various ways to 
solve this problem such as func.bind(). */