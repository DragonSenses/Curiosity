...
In sample-data, let's add a `world-food.csv` file that contains countries and their rice and wheat production in million metric tons.
...
In the `Query Tool` in pgAdmin, add the `CREATE TABLE` query and execute it. 

Then right click the table `world_food`> `Import/Export Data...`.

In `General` tab, make sure the `Import` is selected. Then navigate to the file path of the CSV file. In the `Options` tab, have the `Header` slider selected to ON. Next select the `Columns` tab.

Because there is no column for the `id` for each of these records in `world-food.csv` file.

By omitting the id column in your CSV file, PostgreSQL will use the default value (auto-generated by the sequence) for the id field during the import process.

To ensure that postgreSQL auto-generates the `id` check that the `id` tag is **removed** in the `Columns to import` within the `Columns` tab of the `Import/Export data` window.

The only columns to import are `country`, `rice_production` and `wheat_production`. Now click `OK` and we can refresh our database to see the `world_food` table have the IDs automatically generated.