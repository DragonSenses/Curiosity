...
|-------------------------|----------------|
| + We need to revoke them if care about memory.| - No need to revoke anything. |
|+ Direct access to blob, no "encoding/decoding" | - Performance and memory losses on big `Blob` objects for encoding.|
