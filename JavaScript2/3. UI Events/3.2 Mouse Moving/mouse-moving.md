...
see: `skipping.html` to check out "live" on a teststand below.
...
You can see that very well in the example `mouseout.html`: `<div id="child">` is inside the `<div id="parent">`. There are `mouseover/out` handlers on `#parent` element that output event details.
...
See in directory: `example-mouseout`
...
See `eventDelegation.html` in directory `example-event-delegation` to see this in action. As the mouse travels across the elements of this table, the current one is highlighted.
...
See `eventDelegation2.html` in `example-event-delegation-2` directory for full example with all details. 
